...
领域知识最密集的类型之一。AI 可做结构检查和数据来源检查，L4 估值合理性判断需要老师的 `[fatal_defect]` 规则（如"估值模型假设无理由说明→证据维度上限 5/10"）和 `[pitfall]` 常见分析遗漏点。
...
### Phase 1: 知识卡功能上线（3-4 天）— 复用现有基础设施

- [ ] **扩展 `ANNOTATION_TYPES`**: `fatal_defect` + `benchmark` + `scoring_anchor`（`expert_annotator.py` 1 行 + `expert_freeform_parser.py` 3 行）
- [ ] **新建 `build_knowledge_cards()`**: 将 `ExpertInsights` 自动分层为三层知识卡（`app/calibration/knowledge_cards.py`，~30 行）
- [ ] **Orchestrator 集成**: `_load_knowledge_cards_for_competition()` + 注入 Agent prompt（~20 行）
- [ ] **为 ISEF 和 John Locke 编写完整知识卡示例**: 覆盖 Layer 1 全部 3 项 + Layer 2 至少 1 对标杆 + Layer 3 若干自由笔记，作为老师参考模板
- [ ] **在 `competition_registry.json` 中增加 `knowledge_cards_path` 字段**（可选 — 可直接按文件命名约定匹配）

### Phase 2: 置信度标签 + 标注增强（1-2 天）

- [ ] **Agent prompt 增加 `confidence`/`substitutability` 输出字段**: A2/A3/A4/A5 各 prompt 追加格式要求
- [ ] **`annotation_builder.py` 按置信度分级显示**: FULL/REVIEW/ESCALATE 三种标记样式
- [ ] **验证并完善隐私声明**: README + Gradio UI 添加"学生论文发送至 LLM API"的明确说明

### Phase 3: 反馈闭环（2-3 天）

- [ ] **反馈数据 API**: `POST /api/v1/feedback` 接收老师审核结果
- [ ] **Gradio "老师审核" Tab**: 查看 AI 评审结果并逐条确认/修正
- [ ] **置信度校准统计**: 按 Agent/竞赛类型统计 AI 信心 vs 老师确认率

### Phase 4: 深度增强（持续）

- [ ] **A3 prompt 增强推理链分解**: 思辨型论文的 from→to→missing_step→why_critical（不改数据结构，只增强 prompt）
- [ ] **学生水平自适应**: 按字数/复杂度/往年成绩估级
- [ ] **知识卡自动更新建议**: 当老师多次纠正同一模式时触发
- [ ] **扩展 `data/expert_insights/`**: 覆盖全部 7 种竞赛类型的专家经验示例
