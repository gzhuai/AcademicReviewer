...
| # | 改进项 | 影响 | 优先级 | 工作量 |
|---|--------|------|--------|--------|
| 1 | 校准引擎按竞赛类型切换特征集 | 消除最大偏差源 | P0 | 2-3 天 |
| 2 | Agent 权重按竞赛类型可配置 | 评分公平性 | P0 | 1 天 |
| 3 | A4 加载并使用 style_guides | 语言审查学科差异 | P1 | 1 天 |
| 4 | 校准引擎 FEATURE_TO_CONFIG_MAP 按类型分离 | 配置建议正确性 | P1 | 1 天 |
| 5 | 安全加固（认证+上传限制+隐私声明） | 生产就绪 | P1 | 1 天 |
| 6 | A3 type_hints 从代码移到配置文件 | 代码质量 | P2 | 0.5 天 |
| 7 | 其他技术债务清理 | 可持续性 | P2-P3 | 2 天 |
...
| 改进项 | 说明 | 优先级 |
|--------|------|--------|
| `subprocess` → 直接调用 | `gradio_app.py` 中校准功能用 `subprocess.run` 调 CLI，改为直接调 `run_calibration()` | P3 |
| requirements-lock.txt | 用 `pip freeze` 生成精确版本锁文件 | P2 |
| Changelog | 添加 `CHANGELOG.md`，从 git log 反向生成 | P3 |
| Contributing 指南 | 添加 `CONTRIBUTING.md` 说明开发环境设置、运行测试、提 PR 流程 | P3 |
| 数据库迁移 | 从 `create_all()` 迁移到 Alembic 或至少记录 schema 变更 | P3 |
| 配置热加载 | 检测 JSON 文件修改时间，自动刷新缓存 | P3 |

---

## 执行顺序建议

### 第一批 (本周, ~3 天): 消除偏差的核心改进

1. **改进 1**: 校准引擎特征集按类型切换 (2-3 天)
2. **改进 2**: Agent 权重按类型可配置 (1 天)

### 第二批 (下周, ~2 天): 质量提升

3. **改进 3**: A4 加载 style_guides (1 天)
4. **改进 4**: 校准引擎配置映射按类型分离 (1 天)

### 第三批 (后续, ~2 天): 安全与完善

5. **改进 5**: 安全加固 (1 天)
6. **改进 6**: type_hints 移到配置文件 (0.5 天)
7. **改进 7**: 技术债务清理（按需） (0.5-2 天)
